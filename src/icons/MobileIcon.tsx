...
export function MobileIcon({ title, color, width, height, size, ...props }: SvgIconProps): React.ReactElement {
    return (
        <svg {...props} width={width ?? size ?? 31} height={height ?? size ?? 24} viewBox='0 0 32 24' fill='none' xmlns='http://www.w3.org/2000/svg'>
            <title>{title}</title>
            <path
