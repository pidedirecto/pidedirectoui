...
```ts
import { useIsElementVisible } from '@pidedirecto/ui/hooks';

const elementRefInitializer = useIsElementVisible({
    onVisible: () => {},
});
...
-   `onVisible` a callback that will be called each time the element is visible.
-   `onHidden` a callback that will be called each time the element is hidden.
