...
Have in mind that when persisting the action now have to sync the information to the database, therefore actions now are asynchronous
and can throw errors if there is a problem within the database, so the recommendation is hanlding the errors for actions in persisting stores.
