...
1. If you are calling the same endpoint multiple times in the same page, this hook will cache the response and will return it for both
   api calls, actually the endpoint will be only called once, the endpoints can only be called once per time even if it is trying to be called
   in multiple instances, this can be beneficial if the endpoint is called with the same request, but can have unwanted behaviour if you want to call
   the same endpoint multiple times with different request in the same page. Example
...
function Elements() {
    const [elements] = useSuspenseApi(findElementsApi, { ...request1 });

    return elements.map((element) => <span>{element}</span>);
}

function AnotherElements() {
    // How we are calling the same endpoint twice in the same page, it will return the same response of the first component
    // even if we are calling it with different requests. Try to avoid to call the same endpoint multiple times in the same page.
    const [elements] = useSuspenseApi(findElementsApi, { ...request2 });

    return elements.map((element) => <span>{element}</span>);
}
```

2. You can nest `Suspense` components so having multiple components suspended at the same time. example:
