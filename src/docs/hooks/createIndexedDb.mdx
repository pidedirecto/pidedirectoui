...
-   You may ask what are the difference between `createdIndexedDb` and `createIndexedStore`, well the `createIndexedStore`
    ...
    `createdIndexedDb`, and if you want the components re-render to changes on the indexed db use `createIndexedStore`.

    Have in mind not to access to the same db from `createdIndexedDb` and `createIndexedStore` cause you may have unexpected
    behaviours.
