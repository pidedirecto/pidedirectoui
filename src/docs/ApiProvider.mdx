...
import * as ApiProviderStories from '../stories/ApiProvider.stories';

<Meta title="docs/ApiProvider" />

# ApiProvider

`ApiProvider` component makes you able to configure api calls used inside @pidedirecto/ui library.

```tsx
<ApiProvider api={{ createUiLogEvent: createUiLogEventApi }} >
    <App />
</ApiProvider>
```
...
you can receive this UI log event through the `ApiProvider` via the createUiLogEvent callback.
...
<ArgTypes of={ApiProviderStories} />
