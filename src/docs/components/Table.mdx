...
`tooltbar` prop can receive either a function or directly a `React.Element` value, if a function is passed this function should return a `React.Element` and as argument will receive the selected items if table is selectable.
